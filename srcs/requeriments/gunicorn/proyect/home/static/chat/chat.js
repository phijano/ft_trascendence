window.initializeChat = function () {
  ...
  function displayGameInvitation(data) {
    const template = document
      .querySelector("#gameInvitationTemplate")
      .content.cloneNode(true);
    template.querySelector('[data-content="message"]').textContent =
      data.message;
    ...
    acceptBtn.onclick = () =>
      acceptGameInvitation(data.match_id, data.sender_id);
    declineBtn.onclick = () =>
      declineGameInvitation(data.match_id, data.sender_id);
  ...
  function displayWaiting(data) {
    const template = document.querySelector("#waiting").content.cloneNode(true);
    template.querySelector('[data-content="message"]').textContent =
      data.message;
    boxMessages.appendChild(template);
    scrollToBottom();
  ...
  function handleGameInvitationDeclined(data) {
    const template = document
      .querySelector("#gameResponseTemplate")
      .content.cloneNode(true);
    template.querySelector('[data-content="message"]').textContent =
      data.message;
  ...
  function handleGameInvitationAccepted(data) {
    ...
    const template = document
      .querySelector("#gameStartTemplate")
      .content.cloneNode(true);
  ...
  function openPrivateChat(userId) {
    if (userId === connectedUserMap.get(user)) {
      ...
      console.log(
        `Current user ID: ${connectedUserMap.get(
          user
        )} (type: ${typeof connectedUserMap.get(user)})`
      );
  ...
  function handlePrivateChatRejected(data) {
    const template = document
      .querySelector("#privateChatDeclinedTemplate")
      .content.cloneNode(true);
    template.querySelector('[data-content="message"]').textContent =
      data.message;
    boxMessages.appendChild(template);
  ...
  function displayPrivateChatNotification(data) {
    const template = document
      .querySelector("#privateChatRequestTemplate")
      .content.cloneNode(true);
    template.querySelector('[data-content="username"]').textContent =
      data.username;
  ...
  function handlePrivateChatAccepted(data) {
    // Clone the notification template
    const template = document
      .querySelector("#privateChatAcceptedTemplate")
      .content.cloneNode(true);
    // Set the message
    template.querySelector('[data-content="message"]').textContent =
      data.message;
  ...
  function privateChat(data) {
    ...
    if (chatTitle) {
      ...
      const otherUsername =
        currentUser === data.username ? data.target_username : data.username;
  ...
  function displayChatMessage(message, username, avatar) {
    const template = document
      .querySelector("#receivedMessageTemplate")
      .content.cloneNode(true);
  ...
  function loadMessageHTML(m) {
    const template = document
      .querySelector("#sentMessageTemplate")
      .content.cloneNode(true);
  ...
  function updateConnectedUsersList(users) {
    ...
    users.forEach((userObj, idx, allUsr) => {
      const template = document
        .querySelector("#userItemTemplate")
        .content.cloneNode(true);
      template.querySelector('[data-content="username"]').textContent =
        userObj.username;
      ...
      playBtn.onclick = () => inviteToGame(userObj, allUsr);
  ...
  function inviteToGame(userObject, allUsr) {
    closeModal();
    const myself = allUsr.filter((u) => user == u.username);

    chatSocket.send(
      JSON.stringify({
        type: "game_invitation",
        target_user_id: userObject.id,
      })
    );
    chatSocket.send(
      JSON.stringify({
        type: "waiting",
        target_user_id: myself[0].id,
      })
    );
  }
  ...
  function blockUser(userId) {
    ...
    const username = Array.from(connectedUserMap.keys()).find(
      (u) => connectedUserMap.get(u) === userId
    );
    if (username) {
